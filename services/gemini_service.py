...
def _image_to_part(image_path: str) -> types.Part:
    """Convert image file path to Gemini Part object."""
...
async def analyze_selfie(image_path: str) -> dict:
    ...
    image_part = _image_to_part(image_path)

    response = client.models.generate_content(
        model="gemini-1.5-flash",
        contents=[
            image_part,
            types.Part.from_text(SELFIE_ANALYSIS_PROMPT)
        ]
    )
...
async def analyze_clothing(image_path: str) -> dict:
    ...
    response = client.models.generate_content(
        ...
        contents=[
            image_part,
            types.Part.from_text(CLOTHING_ANALYSIS_PROMPT)
        ]
    )
...
async def generate_outfit_image(
    ...
    response = client.models.generate_images(
        ...
        config=types.GenerateImagesConfig(
            number_of_images=1,
            aspect_ratio="2:3",          # Portrait — ideal for fashion
    ...
    # Return as base64 string for storage / frontend display
